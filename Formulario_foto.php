...
function uploadImage(string $temporalName, string $realName): string
{
...
//    var_dump($_FILES['userfile']['tmp_name']);
   $nameFile = uploadImage($_FILES['userfile']['tmp_name'], $_FILES['userfile']['name']);
   var_dump($nameFile);
    ...
    if (!empty($_POST['hobbies'])) {
        ...
        foreach ($_POST['hobbies'] as $hobby) {
            $cleanedHobby = filter_var($hobby, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    echo '<img src = "/uploads/'.$nameFile.'" alt="imagen" width="300" height="100">';
} else {
    $form_html = <<<HTML
